...
#***************************___04_1_Reduced_VAR_Model__************************#

#******************************__Create_VAR_Model__****************************#

# Create VAR model matrix
var_model_a <- cbind( v_CR, v_IPI, v_CCI, v_sa_core_CPI )
colnames(var_model_a) <- c( "v_CR", "v_IPI", "v_CCI", "v_sa_core_CPI")
...
delay_2019 <- VARselect(var_model_a["::2019-12-31"],
                        type ="const",  # Constant term included
                        lag.max = 10,
                        exogen = NULL,
                        season = NULL)
summary(delay_2019)
...
delay_2023 <- VARselect(var_model_a,
                        type ="const",  # Constant term included
                        lag.max = 10,
                        exogen = NULL,
                        season = NULL)
...
var_model_result_2019 <- VAR(var_model_a["::2019-12-31"], p = 5, type = "const")
# Estimate VAR model using all available data
var_model_result_2023 <- VAR(var_model_a, p = 5, type = "const")
...
# Create a list of VAR models for different years
...
for (date_filter in names(filtered_var_model)) {
  ...
  plot(time_range, cumulative_irf, type = "l", lwd = 2,
       ...
       main = paste("CIR of CR to sa_core_CPI -", date_filter, ", ", "lag = ", var_model_results$p),
       xaxt = "n",
